    ...
    else
    {
        ...
        if (! inserted)
        {
            helpers::getLogLog().error(
                DCMTK_LOG4CPLUS_TEXT("Hierarchy::getInstanceImpl()- Insert failed"),
                true);
        }
        ...
        if (pnm_it != provisionNodes.end())
        {
            ...
            {
                helpers::getLogLog().error(
                    DCMTK_LOG4CPLUS_TEXT("Hierarchy::getInstanceImpl()- Delete failed"),
                    true);
            }
        }
        ...
        i = name.find_last_of(DCMTK_LOG4CPLUS_TEXT('.'), i-1)) 
    {
        ...
        if(it != loggerPtrs.end()) {
            ...
            if(it2 != provisionNodes.end()) {
                ...
                //bool inserted = provisionNodes.insert(OFMake_pair(substr, node)).second;
                if(!tmp.second) {
                    helpers::getLogLog().error(
                        DCMTK_LOG4CPLUS_TEXT("Hierarchy::updateParents()- Insert failed"),
                        true);
                }
            }
        } // end if Logger found
